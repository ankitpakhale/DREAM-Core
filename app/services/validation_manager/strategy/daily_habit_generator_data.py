...
class DailyHabitGeneratorData(BaseStrategy):
    _instance = None

    def __new__(cls, *args, **kwargs):
        if cls._instance is None:
            cls._instance = super(DailyHabitGeneratorData, cls).__new__(
                cls, *args, **kwargs
            )
    ...
    def _process_payload(payload: Dict[str, str]) -> Dict[str, bool]:
        ...
        logger.debug(
            f"{PAYLOAD_CATEGORY.DAILY_HABIT_GENERATOR_DATA.replace("_", " ").title()} processed successfully!!!"
        )
        return payload
    ...
    def _validate(self, payload: Dict[str, str], schema: Dict) -> None:
        ...
        logger.debug(
            f"{PAYLOAD_CATEGORY.DAILY_HABIT_GENERATOR_DATA.replace("_", " ").title()} validated successfully!!!"
        )

    def validate(self, payload: Union[list, dict]) -> None:
        schema = SCHEMA_MAP[PAYLOAD_CATEGORY.DAILY_HABIT_GENERATOR_DATA]
        return self._validate(payload=payload, schema=schema)


# singleton instance of DailyHabitGeneratorData
daily_habit_generator_data = DailyHabitGeneratorData()
